...
void WriteEncodedFile(std::string & fileName, std::string & outputFileName){
    ...
    unsigned int linepos =0 ; // current io positions, always < base64::MAX_BASE64_WIDTH
    unsigned int n_base64_char; // total base64 char obtained
    char rawBuffer[base64::IO_CHUNK]; // buffer to store raw io data
    char encodedBuffer[base64::encoded_size(base64::IO_CHUNK)]; // buffer for converted data
    ...
    while (inputFile.read(rawBuffer, base64::IO_CHUNK) || inputFile.gcount()){
        n_base64_char = base64::encode(encodedBuffer, rawBuffer, inputFile.gcount());
...
int main(int argc, char * argv[]){
    if ((argc < 2) || (argc > 4)){
        std::cerr << "IMPROPER ARGUMENTS, please provide input and output file names or options\n";
        print_help();
        return 1;
    }
